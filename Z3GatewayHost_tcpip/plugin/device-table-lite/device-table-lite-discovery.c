...
#define newDeviceEventControl emberAfPluginDeviceTableLiteNewDeviceEventControl
...
void emberAfPluginDeviceTableLiteIndexAddedCallback(uint16_t index);

// Bookkeeping callbacks
void emAfPluginDeviceTableLiteDeviceLeftCallback(EmberEUI64 newNodeEui64);
...
static void newEndpointDiscovered(EmberAfPluginDeviceTableLiteEntry *p_entry)
{
  ...
  // has joined.
  emberAfPluginDeviceTableLiteNewDeviceCallback(p_entry->eui64);
  emAfDeviceTableSave();
}
...
void emberAfPluginDeviceTableLiteNewDeviceEventHandler(void)
{
  ...
  EmberStatus status;
  uint8_t taskQueueIndex;
  uint8_t nextState = DEVICE_DISCOVERY_STATE_ENDPOINTS_WAITING; 
  uint8_t delay = DEVICE_DISCOVERY_ENDPOINT_RETRY_TICKS;
                                                  ...
                                                  uint16_t length)
{
  ...
  if (i != DEVICE_QUEUE_NULL) {
    ...
    entry.delay = 0;
    entry.waitTime = 0;
    epCnt = message[ACTIVE_ENDPOINT_RESPONSE_COUNT_OFFSET];
    emberAfCorePrintln("number of ep: %d", epCnt);
    epCnt = (epCnt > DEVICE_MAX_ENDPOINTS) ? DEVICE_MAX_ENDPOINTS : epCnt;
    for (i = 0; i < epCnt; i++) {
      entry.endpoint = 1 + i;
      emberAfCorePrintln("ep: %d", 1 + i);
...
{
  uint8_t endpoint;
  EmberAfPluginDeviceTableLiteEntry *pEntry;
  ...
  if (endpointIndex == EMBER_AF_PLUGIN_DEVICE_TABLE_NULL_INDEX) {
    ...
    emberAfPluginDeviceTableLiteIndexAddedCallback(endpointIndex);
  }
                                            ...
                                            EmberEUI64 newNodeEui64)
{
  ...
  uint16_t i;
  EmberAfPluginDeviceTableLiteEntry *deviceTable = emberAfDeviceTablePointer();
  DeviceTableQueueEntry entry;
  ...
  if (deviceTableIndex == EMBER_AF_PLUGIN_DEVICE_TABLE_NULL_INDEX) {
    ...
    if (newNodeId != deviceTable[deviceTableIndex].nodeId) {
      ...
      // New device is set, time to make the callback to indicate a new device
      // has joined.
      emberAfPluginDeviceTableLiteRejoinDeviceCallback(deviceTable[deviceTableIndex].eui64);
...
static void newDeviceLeftHandler(EmberEUI64 newNodeEui64)
{
  ...
  if (index != EMBER_AF_PLUGIN_DEVICE_TABLE_NULL_INDEX) {
    emAfPluginDeviceTableLiteDeviceLeftCallback(newNodeEui64);
 ...
 */
bool emAfPluginDeviceTableLitePreZDOMessageReceived(EmberNodeId emberNodeId,
                                                EmberApsFrame* apsFrame,
                                                uint8_t* message,
                                                uint16_t length)
{
