...
static void forwardCommand(
  ...
  EmberAfPluginCommandRelayLiteDeviceEndpoint* deviceEndpoint);
static bool matchDeviceEndpoints(
  EmberAfPluginCommandRelayLiteDeviceEndpoint* deviceEndpoint1,
  EmberAfPluginCommandRelayLiteDeviceEndpoint* deviceEndpoint2);
...
static EmberAfPluginCommandRelayLiteEntry relayTable[EMBER_AF_PLUGIN_COMMAND_RELAY_LITE_RELAY_TABLE_SIZE];
...
bool emAfPluginCommandRelayLitePreCommandReceivedCallback(EmberAfClusterCommand* cmd)
{
  EmberAfPluginCommandRelayLiteDeviceEndpoint deviceEndpoint;
  ...
  for (i = 0; i < EMBER_AF_PLUGIN_COMMAND_RELAY_LITE_RELAY_TABLE_SIZE; i++) {
...
void emberAfPluginCommandRelayLiteLoad(void)
{
  ...
  while ((getline(&lineBuf, &lineBufSize, fp) != -1)
         && index < EMBER_AF_PLUGIN_COMMAND_RELAY_LITE_RELAY_TABLE_SIZE) {
    if (sscanf(lineBuf,
...
static void removeRelayEntry(uint16_t relayTableIndex)
{
  if (relayTableIndex > EMBER_AF_PLUGIN_COMMAND_RELAY_LITE_RELAY_TABLE_SIZE) {
    return;
  }
